...
#	composer remove prestashop/autoindex
	...
	cp -r prestashop-partner-tracking/* shoparizepartner
...
ifeq ($(COMPOSERV1),1)
	...
	cd $(ROOT_DIR) && composer remove prestashop/autoindex && composer install --ignore-platform-reqs --no-ansi --no-interaction --no-scripts --no-progress --prefer-dist
